...
class CustomLoginView(LoginView):
    ...
    success_url = reverse_lazy("authentication:service_home")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if login_type == "admin":
            admin_form = self.admin_form(request.POST, request=request)
            user_form = self.user_form()
            active_form = admin_form
            active_tab = "admin"
        else:
            user_form = self.user_form(request.POST, request=request)
            admin_form = self.admin_form()
